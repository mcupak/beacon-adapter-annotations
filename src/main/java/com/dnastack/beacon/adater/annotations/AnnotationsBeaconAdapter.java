...
public class AnnotationsBeaconAdapter implements BeaconAdapter {
    ...
    private BeaconDatasetAlleleResponse getDatasetResponse(String datasetId, String assemblyId, String referenceName,
        ...
        List<Variant> variants = loadVariants(variantSet.getId(), referenceName, start)
                .stream()
    ...
    private Double calculateFrequency(String alternateBases, List<Variant> variants) {
        ...
        Long totalGenotypesCount = variants
                ...
                .flatMap(variant -> variant
                        .getCallsList()
                        .stream())
                .collect(Collectors.summingLong(Variants.Call::getGenotypeCount));
    ...
    private long calculateMatchingGenotypesCount(Variant variant, String alternateBases) {
        ...
        return variant
                .getCallsList()
                .stream()
                .map(Variants.Call::getGenotypeList)
                .flatMap(List::stream)
    ...
    private VariantSet loadVariantSet(String id) throws BeaconAlleleRequestException {
        try {
            ...
            BeaconAlleleRequestException alleleRequestException =
                    new BeaconAlleleRequestException(String.format("Couldn't load variant set for id %s.", id), Reason.CONN_ERR, null);
            alleleRequestException.initCause(e);
            throw alleleRequestException;
        }
    }
    ...
    private VariantAnnotationSet loadVariantAnnotationSet(String id) throws BeaconAlleleRequestException {
        try {
            ...
            BeaconAlleleRequestException alleleRequestException =
                    new BeaconAlleleRequestException(String.format("Couldn't load variant annotation set for id %s.", id), Reason.CONN_ERR, null);
            alleleRequestException.initCause(e);
            throw alleleRequestException;
        }
    }
